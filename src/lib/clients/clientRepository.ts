...
export function watchClients(cb: (rows: Client[]) => void) { return onSnapshot(query(collection(assertDb(), COLLECTION), orderBy("fullName", "asc")), (s) => cb(s.docs.map((d) => adaptClientDoc(d.id, d.data() as Record<string, unknown>)))); }
