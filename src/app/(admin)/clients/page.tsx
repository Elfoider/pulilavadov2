...
const empty: ClientFormValues = { fullName: "", phone: "", documentId: "", address: "", observations: "" };
...
export default function ClientsPage() {
  ...
  return <div className="space-y-4">
    ...
    {open && <div className="rounded-xl border bg-white p-4"><ClientForm values={values} onChange={setValues} submitLabel={editingId?"Guardar":"Crear"} onCancel={()=>setOpen(false)} onSubmit={()=>{const now=new Date().toISOString(); if(editingId){setClients(prev=>prev.map(c=>c.id===editingId?{...c,...values,updatedAt:now}:c));} else {setClients(prev=>[{id:crypto.randomUUID(),createdAt:now,updatedAt:now,...values},...prev]);} setOpen(false);}}/></div>}
    <div className="overflow-x-auto rounded-xl border bg-white"><table className="min-w-full text-sm"><thead className="bg-slate-50"><tr><th className="px-3 py-2 text-left">Nombre</th><th className="px-3 py-2 text-left">Teléfono</th><th className="px-3 py-2 text-left">Cédula/RIF</th><th className="px-3 py-2 text-left">Origen</th><th className="px-3 py-2 text-left">Acciones</th></tr></thead><tbody>{filtered.map(c=><tr key={c.id} className="border-t"><td className="px-3 py-2">{c.fullName}</td><td className="px-3 py-2">{c.phone}</td><td className="px-3 py-2">{c.documentId}</td><td className="px-3 py-2">{c.legacy ? "Viejo" : "Nuevo"}{c.legacy ? <span className="ml-2 rounded bg-amber-100 px-2 py-0.5 text-xs text-amber-700">Dato antiguo</span> : null}</td><td className="px-3 py-2 flex gap-2"><button className="rounded bg-slate-200 px-2 py-1" onClick={()=>{setValues({fullName:c.fullName,phone:c.phone,documentId:c.documentId,address:c.address??"",observations:c.observations??""});setEditingId(c.id);setOpen(true);}}>Editar</button><button className="rounded bg-rose-100 px-2 py-1 text-rose-700" onClick={()=>setClients(prev=>prev.filter(x=>x.id!==c.id))}>Eliminar</button><button className="rounded bg-blue-100 px-2 py-1 text-blue-700" onClick={()=>setHistoryClientId(c.id)}>Historial</button></td></tr>)}</tbody></table></div>
